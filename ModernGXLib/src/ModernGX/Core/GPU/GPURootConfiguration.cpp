...
MGX::Core::GPU::RootConfigurationEntry& MGX::Core::GPU::RootConfiguration::operator[](unsigned int index)
{
    // Check bounds
    if (index >= m_usage)
    {
...
bool MGX::Core::GPU::RootConfiguration::Bind(ID3D12GraphicsCommandList* ptrCmdList)
{
...
void MGX::Core::GPU::RootConfiguration::__bindHelper(ID3D12GraphicsCommandList* ptrCmdList, unsigned int index, const RootConfigurationEntry* ptrEntry)
{
    // Swtich on type
    switch (ptrEntry->type)
    {
        ...
        case RootConfigurationEntry_t::Invalid:
            __fallthrough; // MSVC no warning helper
        default:
            break;
    }
}
